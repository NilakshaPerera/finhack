...
class SessionController extends Controller {
    ...
    public function session(Request $request) {
        ...
        if (!$currectSession) {
            ...
            $presentation = null;
            if( Setting::where('setting' , 'assign_dummy_presentation')->first()->data){
                $presentation = Presentation::where('dummy', 1 )->first();
            }else{
                //  get random presentations
                $presentation = Presentation::where('id', Auth::user()->presentation_id )->first();
            }
