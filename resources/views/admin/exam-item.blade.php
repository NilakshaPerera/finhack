...
<div class="col-md-12 col-sm-12 col-xs-12">
    <div class="x_panel">
        <div class="container-custom">
          ...
          <h3><a href="javascript:void(0)" data-toggle="modal" data-target="#myModal1">  {{ $caseStudy->name }} </a></h3>
            </br>
            <?php
            if ($mcqs) {
                foreach ($mcqs as $mcq) {
                ...
                <?php } else { ?>
                    <div>
                        <?php
                            ...
                            if ($temp !== 'mp4') {
                                ?>
                            <a class="badge" target="_blank" href="{{ url('/') .'/uploads/video-answers/' . $exam->presentation['vid'] }}">Mind Map Answer Attachment</a>
                            <br>
                        <?php } else { ?>
                            <video controls autoplay="autoplay">
                                <source src="{{ url('/') .'/uploads/video-answers/' . $exam->presentation['vid'] }}" type="video/mp4">
        ...
        <form id="formadduser" name="formadduser" data-parsley-validate="" class="form-horizontal form-label-left" novalidate="" method="POST" action="{{ route('admin/score') }}">
            ...
            <div class="form-group">
                ...
                <?php if(Auth::user()->role_id == 1){ ?>
                    ...
                    <?php }else{ ?>
                        <input placeholder="Presentation Score" type="number" id="presentation-score" name="presentation-score" required="required" value="{{ ($exam->presentation_score == null)? '': $exam->presentation_score }}" class="form-control col-md-7 col-xs-12" disabled>
                        <?php } ?>
                </div>
            </div>
            <div class="ln_solid"></div>
  ...
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <button type="button" class="close" data-dismiss="modal">&times;</button>
        <h4 class="modal-title">{{$caseStudy->name}}  </h4>
